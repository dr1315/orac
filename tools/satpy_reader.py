...
def read_sat_data(fname, sensor = 'fci', x0 = None, x1 = None, y0 = None, y1 = None):
    ...
    rel_azi = np.abs(sat_azi - sol_azi)
    # Clean it up
    ...
    all_channel_sw_flag = tuple([1. if _ < 3.7 else 0. for _ in channel_dim])
    all_channel_lw_flag = tuple([1. if _ >= 3.7 else 0. for _ in channel_dim])
    all_channel_ids_rttov_coef_sw = tuple([n + 1. if _ < 3.7 else 0 for n, _ in enumerate(channel_dim)])
    all_channel_ids_rttov_coef_lw = tuple([n + 1 - sum(all_channel_sw_flag) if _ >= 3.7 else 0 for n, _ in enumerate(channel_dim)])
